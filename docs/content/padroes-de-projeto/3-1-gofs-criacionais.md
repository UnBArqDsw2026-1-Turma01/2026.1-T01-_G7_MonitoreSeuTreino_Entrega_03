...
| `User.reconstitute()`                       | Factory de reconstitution — restaura o estado persistido sem gerar novos IDs ou disparar eventos de negócio.          | `src/domain/entities/user.entity.ts`                               |
...
const refreshToken = RefreshToken.create(user.id, tokenHash, expiresAt); // Sem eventos na criação estrutural do token
...
| Endpoints afetados                | POST `/v1/auth/signup` (avalia `User.create()`) e POST `/v1/auth/login` (avalia `RefreshToken.create()`).                                          |
...
- **Blindagem em nível de compilação**: O bloqueio físico do construtor remove em definitivo a possibilidade de programadores instanciarem entidades de maneira inconsistente ou inadequada fora dos padrões estabelecidos da equipe.
- **Transparência e expressividade no código**: Métodos como `User.create()` e `RefreshToken.reconstitute()` removem ambiguidades de leitura, expondo claramente no código a intenção de negócio aplicada.
...
- **Silenciamento total de dados inconsistentes**: O método `reconstitute()` assume por design que os dados extraídos da base de persistência são válidos e confiáveis. Caso haja dados históricos corrompidos ou inconsistentes salvos anteriormente, o método não aplicará novas regras de validação dinâmicas, delegando essa consistência de legado à gestão rigorosa de migrações no banco.

##### Alternativas consideradas

- **Construtor único parametrizado por Flag (`isNew`)**: Avaliado e sumariamente descartado por ferir a legibilidade do código (parâmetros booleanos obscuros) e transferir indevidamente a responsabilidade de gerenciar o fluxo interno para os chamadores na aplicação.
- **Segregação em duas classes (`NewUser` e `ExistingUser`)**: Cogitado para garantir tipos separados no compilador. Contudo, foi descartado no escopo atual devido à duplicação excessiva de código e complexidade nas assinaturas de retorno de repositórios e barramentos, visto que o comportamento operacional de pós-criação de ambos os cenários é idêntico.
