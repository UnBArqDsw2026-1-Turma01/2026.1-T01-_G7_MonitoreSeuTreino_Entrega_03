...
1. **Herança**: `CachingUserPostgresRepository extends UserPostgresRepository`. Problema: acopla o cache à implementação concreta de Postgres; combinar cache _e_ log por herança exigiria uma terceira classe — impossível em TypeScript sem mixins.
...
- **Herança com mixin**: criaria uma classe monolítica e impossibilitaria compor apenas cache _ou_ apenas log de forma independente. Rejeitado.
