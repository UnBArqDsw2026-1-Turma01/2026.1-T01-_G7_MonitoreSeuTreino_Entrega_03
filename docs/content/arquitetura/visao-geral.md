...
| ------------ | ------------------------------------------------- | ---------------------------------------------- | ------------ |
| Autenticação | `auth/` (JWT, refresh token, eventos, decorators) | `features/auth/` (login, cadastro)             | Implementado |
...
Os padrões mapeados nesta entrega foram aplicados de forma distribuída nos módulos de **Onboarding** e **Autenticação**, reforçando a estrutura arquitetural. A tabela abaixo localiza cada padrão na arquitetura:
