...
export class CloneRoutineUseCase extends UseCase<CloneRoutineInput, void> {
  ...
  protected async handle(input: CloneRoutineInput): Promise<void> {
    ...
    const originalRoutine = await this.routineRepository.findById(input.routineId);
    ...
    if (originalRoutine.userId.toString() !== input.userId) {
      throw new ValidationException('You do not have permission to clone this routine');
    }

    const newRoutineName = input.newName ? RoutineName.create(input.newName) : undefined;
